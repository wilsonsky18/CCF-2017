...
create table if not exists jpc_wif_not_null_train as
...
drop table if exists jpc_wif_notnull_test ;
create table if not exists jpc_wif_not_null_test as
select * from jpc_test where signal != 'null';

drop table if exists jpc_wifi_notnull_test_temp;
create table if not exists jpc_wifi_notnull_test_temp as
select row_id, shop_id,wifi_id,num,row_number() over (partition by row_id order by num desc) rank_wifi from(
select temp2.row_id,temp1.wifi_id,temp1.shop_id,temp1.num from jpc_wifi_train temp1 join jpc_wif_notnull_test temp2 on temp1.wifi_id=temp2.wifi_id) temp3;
